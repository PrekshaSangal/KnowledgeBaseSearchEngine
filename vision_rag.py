...
st.set_page_config(layout="wide", page_title="VisRAG")
st.title("VisRAG (Visual Retrieval-Augmented Generation)")

# --- API Key Input ---
load_dotenv()
with st.sidebar:
    st.header("🔑 API Keys")
    cohere_api_key = st.text_input("Cohere API Key", type="password", key=os.getenv("COHERE_API_KEY"))
    google_api_key = st.text_input("Google API Key (Gemini)", type="password", key=os.getenv("GOOGLE_API_KEY"))
